...
WITH arrival AS (-- CTE to create on_time flag and late_flag 
		...
		,	CASE WHEN order_delivered_customer_date < order_estimated_delivery_date THEN 1 ELSE 0 END AS On_time_flag
		FROM
			purch 
	)
...
,	excl_otr AS (-- filter to exclude the 1st sale from calculating seller on-time rate
		...
		FROM roll_otr
		WHERE
			tot_deliveries >0 -- drops 1st order 
	)
...
,	otr.seller_id
,   s.seller_state	
,	vol.tot_sales AS Seller_volume
, 	ar.on_time_flag
,	ar.late_flag
,	(100.0 * otr.on_time_deliveries /otr.tot_deliveries) AS rolling_otr
,	ao.tot_items
,	ao.tot_weight
,	ao.tot_volume
,	ao.tot_value
...
JOIN 
	excl_otr otr
ON ar.order_id = otr.order_id AND ar.timestamp = otr.timestamp
LEFT JOIN
	states s
ON ar.order_id = s.order_id 
JOIN
	excl_vol vol
ON ar.order_id = vol.order_id AND ar.timestamp = vol.timestamp
;